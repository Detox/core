...
this.a.destroy()},ca:function(){return!this.C&&!!(this.c.size<aa||this.aa(!0).length)},aa:function(a){null==a&&(a=!1);var b=[];var f=+new Date-1E3*Ga;var d=!1;this.c.forEach(function(h,k){!d&&h<f&&(b.push(k),a&&!d&&(d=!0))});return b},ia:function(){var a,b;if(a=this.O(5)){var f=0;for(b=a.length;f<b;++f){var d=a[f];this.$(d)}}},$:function(a){this.H.add(a);this.J(a,Y,new Uint8Array(0))},U:function(a,b){null==b&&(b=[]);var f=this.O(1,b);return f?(a=this.N(a-1,b.concat([f])))?f.concat(a):null:null},O:function(a,
b){var f,g,h=[];null==a&&(a=1);null==b&&(b=[]);if(!this.l.size)return this.b.lookup(p(d)),null;var k=Array.from(this.l.values());var l=0;for(g=(f=this.get_bootstrap_nodes()).length;l<g;++l){var c=f[l];b.push(Fa(c.node_id))}var m=A(b);k=k.filter(function(a){return!m.has(a)});if(!k.length)return null;for(l=0;l<a;++l)k.length&&h.push(N(k));return h},N:function(a,b){var f=[];if(this.c.size<a)return null;var d=Array.from(this.c.keys());if(b){var h=A(b);d=d.filter(function(a){return!h.has(a)})}if(d.length<
a)return null;for(b=0;b<a;++b)f.push(N(d));return f},V:function(a,b,d,g){var f=t([d,g]);if(!this.g.has(f)){var k=t([a,b]);this.h.set(k,[d,g]);this.g.set(f,[a,b]);this.M.set(k,x.Multiplexer(this.I,this.ba));this.L.set(k,x.Demultiplexer(this.I,this.ba));this.fire("routing_paths_count",this.h.size)}},W:function(a,b){var d=this;var g=t([a,b]);if(this.j.has(g)&&(this.j["delete"](g),this.a.destroy_routing_path(a,b),this.w["delete"](g),this.m.forEach(function(a,b){var c=a[0];var f=a[1];a=a[2];c=t([c,f]);
